...
NTSTATUS EnumTdxAddress(WDFREQUEST Request, size_t InputBufferLength, size_t OutputBufferLength, size_t* Resquestlength)
{
	if (!OutputBufferLength)
		return STATUS_BUFFER_TOO_SMALL;
	UNICODE_STRING mou;
	PDRIVER_OBJECT DriverObject = NULL; //键盘的驱动对象，保存这个是为了在卸载时还原它的分发函数
	RtlInitUnicodeString(&mou, L"\\Driver\\Tdx");
