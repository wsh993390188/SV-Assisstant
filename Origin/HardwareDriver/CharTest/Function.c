...
)
{
	...
	else
	{
		...
		__try
		{
			MSR_Data.Value = __readpmc(MSR_Data.msr_address);
		}
		__except (EXCEPTION_EXECUTE_HANDLER)
		{
			WdfRequestComplete(Request, STATUS_UNSUCCESSFUL);
...
NTSTATUS __stdcall SMBUS_Read_Byte(size_t Base_Address, size_t Slave_Address, size_t Offset, OUT PUCHAR Data)
{
	...
	temp = READ_PORT_UCHAR((PUCHAR)(USHORT)(Base_Address + 2));

	temp = READ_PORT_UCHAR((PUCHAR)(USHORT)(Base_Address + 8));
