...
#pragma alloc_text (PAGE, SystemInfoEvtDriverContextCleanup)
#pragma alloc_text (PAGE, SystemInfoEvtDriverUnload)
#endif
...
--*/
{
	...
	attributes.EvtCleanupCallback = SystemInfoEvtDriverContextCleanup;
	...
	//
	config.EvtDriverUnload = SystemInfoEvtDriverUnload;
	...
	// software device.
	//
	status = SystemInfoDeviceAdd(hDriver, pInit);

	return status;
}

VOID
SystemInfoEvtDriverContextCleanup(
