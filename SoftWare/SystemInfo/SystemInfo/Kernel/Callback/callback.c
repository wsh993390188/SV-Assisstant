	...
	_Out_opt_ size_t* Resquestlength)
{
	...
	*Resquestlength = PSP_MAX_CREATE_PROCESS_NOTIFY * sizeof(DWORD64);
	...
	_Out_opt_ size_t* Resquestlength)
{
	...
	RtlZeroMemory(buffer, OutputBufferlength);
	*Resquestlength = PspCreateThreadNotifyRoutine_Count * sizeof(DWORD64);
	...
	_Out_opt_ size_t* Resquestlength)
{
	...
	RtlZeroMemory(buffer, OutputBufferlength);
	*Resquestlength = PspCreateLoadImageNotifyRoutine_Count * sizeof(DWORD64);
	...
	_Out_opt_ size_t* Resquestlength)
{
	...
	if (!NT_SUCCESS(status))
	{
	   ...
	   RtlZeroMemory(buffer, OutputBufferlength);
	*Resquestlength = CmpCallBack_Count * sizeof(DWORD64);
	...
	_Out_opt_ size_t* Resquestlength)
{
	...
	RtlZeroMemory(buffer, OutputBufferlength);
	*Resquestlength = KeBugCheck_Count * sizeof(BUG_STRUCT);
	...
	_Out_opt_ size_t* Resquestlength)
{
	...
	if (!NT_SUCCESS(status))
	{
	  ...
	  RtlZeroMemory(buffer, OutputBufferlength);
	*Resquestlength = KeBugCheck_Count * sizeof(SHUTDOWN_STRUCT);
