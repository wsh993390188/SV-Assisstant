...
{
	for (size_t i = 0; i < this->UsbHostControllerInfo.size(); ++i)
	{
...
BOOL _EnumUsbHub::Updatedata()
{
	this->Device_Hub_Tmp = {};
	this->Device_Tmp.clear();
	this->Root_Hub_Tmp.clear();
	this->Hub_tmp = {};
	this->Usb_Tree.clear();
...
BOOL _EnumUsbHub::EnumUsbPort(HANDLE hHubDevice, ULONG PortNums, bool IsRootHub)
{
	...
	for (ULONG Index = 1; Index <= PortNums; ++Index)
	{
		...
		if (ConnectionInfo.ConnectionStatus != NoDeviceConnected)
		{
			...
			{
				success = this->DriverNametoDevInstance(DriverKeyName, DevProps, DeviceBDF);
...
// 			if (success == TRUE)
// 			{
// 				OutputDebugString(L"直接找到InterFace Descriptor!\n");
// 			}
		}
...
BOOL _EnumUsbHub::GetExternalHubName(HANDLE Hub, ULONG ConnectionIndex, wstring & ExternalHubName)
{
	...
	//
	ExternalHubName = extHubNameW->NodeName;
...
BOOL _EnumUsbHub::GetDriverKeyName(HANDLE Hub, ULONG ConnectionIndex, wstring & DriverKeyName)
{
	...
	//
	DriverKeyName = driverKeyNameW->DriverKeyName;
...
BOOL _EnumUsbHub::GetPortConnectorProps(HANDLE hHubDevice, ULONG Index, USB_PORT_CONNECTOR_PROPERTIES_V2& PortConnectorProps)
{
	...
	if (!success)
	{
		OutputDebugPrintf(_T("IOCTL_USB_GET_PORT_CONNECTOR_PROPERTIES ERROR Get Length, ERROR Code: %X\n"), GetLastError());
		return GetLastError();
	}
	...
	{
		OutputDebugPrintf(_T("Incorrect length received by IOCTL USB GET PORT CONNECTOR PROPERTIES\n"));
		return FALSE;
	}
	...
	{
		OutputDebugPrintf(_T("IOCTL_USB_GET_PORT_CONNECTOR_PROPERTIES ERROR Get Info, ERROR Code: %X\n"), GetLastError());
	...
	PortConnectorProps.CompanionHubSymbolicLinkName = pPortConnectorProps->CompanionHubSymbolicLinkName;
	if (pPortConnectorProps)
	{
		HeapFree(GetProcessHeap(), HEAP_ZERO_MEMORY, pPortConnectorProps);
		pPortConnectorProps = NULL;
	...
	vector<USB_ALL_DESCRIPTOR>& AllDescs)
{
	...
	if (DeviceDesc.iManufacturer)
	{
		if (this->GetStringDescriptor(hHubDevice,
			ConnectionIndex,
			DeviceDesc.iManufacturer,
			0x0409,
			TmpDesc) == ERROR_SUCCESS)
			StringDesc.emplace_back(TmpDesc);
		else
		{
			StringDesc.emplace_back(L"String Descriptor not available");
		}
	}
	TmpDesc.clear();
