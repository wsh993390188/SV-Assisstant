...
std::wstring GetDeviceIDFromPhysicalDriveID(const INT physicalDriveId, const BOOL IsKernelVerEqualOrOver6)
{
	...
	const std::wstring query2findcontroller = L"ASSOCIATORS OF {Win32_PnPEntity.DeviceID='%1%'} WHERE AssocClass=Win32_SCSIControllerDevice";
	std::wstring query;
	std::wstring result;
	...
	try
	{
			...
			IID_IWbemLocator, (LPVOID *)&pIWbemLocator)))
		{
				...
				NULL, NULL, 0L, securityFlag, NULL, NULL, &pIWbemServices)))
			{
				...
				{
					query = boost::str(boost::wformat(query2findstorage.c_str()) % physicalDriveId);
					std::wstring StorageID = GetStringValueFromQuery(pIWbemServices, query, L"DeviceID");
					query = boost::str(boost::wformat(query2findcontroller.c_str()) % StorageID);
					std::wstring ControllerID = GetStringValueFromQuery(pIWbemServices, query, L"DeviceID");
					result = ControllerID;
				}
			}
		}
...
SlotMaxCurrSpeed GetSlotMaxCurrSpeedFromDeviceID(const std::wstring DeviceID)
{
...
{
	std::wstring result = boost::str(boost::wformat(L"PCIe %d.0 x%d") % speedtoconv.SpecVersion %speedtoconv.LinkWidth);
	return result;
}