...
public:
	...
	struct ATA_SMART_INFO
	{
		...
		std::string			SerialNumberReverse;
		std::wstring		FirmwareRev;
		std::string			FirmwareRevReverse;
		std::wstring		Model;
		std::string			ModelReverse;
		std::wstring		ModelWmi;
		std::wstring		ModelSerial;
		std::wstring		DriveMap;
	...
	std::vector<ATA_SMART_INFO> vars;
	std::vector<EXTERNAL_DISK_INFO> externals;

	std::vector<std::wstring> m_IdeController;
	std::vector<std::wstring> m_ScsiController;
	std::vector<std::wstring> m_UsbController;
	std::wstring m_ControllerMap;
	std::vector<std::wstring> m_BlackIdeController;
	std::vector<std::wstring> m_BlackScsiController;
	std::vector<std::wstring> m_SiliconImageController;
	std::vector<std::wstring> m_UASPController;
	std::vector<DWORD> m_SiliconImageControllerType;
	std::vector<INT> m_BlackPhysicalDrive;
...
protected:
	...
	std::wstring m_SerialNumberA_Z[26];
	BOOL m_FlagAtaPassThrough;
	BOOL m_FlagAtaPassThroughSmart;
	BOOL m_FlagNVMeStorageQuery;

	BOOL GetDiskInfo(INT physicalDriveId, INT scsiPort, INT scsiTargetId, INTERFACE_TYPE interfaceType, COMMAND_TYPE commandType, VENDOR_ID vendorId, DWORD productId = 0, INT scsiBus = -1, DWORD siliconImageId = 0, BOOL FlagNvidiaController = 0, BOOL FlagMarvellController = 0, std::wstring pnpDeviceId = _T(""));
	BOOL AddDisk(INT PhysicalDriveId, INT ScsiPort, INT scsiTargetId, INT scsiBus, BYTE target, COMMAND_TYPE commandType, IDENTIFY_DEVICE* identify, INT siliconImageType = -1, PCSMI_SAS_PHY_ENTITY sasPhyEntity = NULL, std::wstring pnpDeviceId = _T(""));
	...
	BOOL AddDiskNVMe(INT PhysicalDriveId, INT ScsiPort, INT scsiTargetId, INT scsiBus, BYTE target, COMMAND_TYPE commandType, IDENTIFY_DEVICE* identify, std::wstring pnpDeviceId = _T(""));
	...
	DWORD GetTransferMode(WORD w63, WORD w76, WORD w77, WORD w88, std::wstring &currentTransferMode, std::wstring &maxTransferMode, std::wstring &Interface, INTERFACE_TYPE *interfaceType);
	VOID GetTransferModePCIe(std::wstring & current, std::wstring & max, SlotMaxCurrSpeed slotspeed);
	DWORD GetTimeUnitType(std::wstring model, std::wstring firmware, DWORD major, DWORD transferMode);
	DWORD GetAtaMajorVersion(WORD w80, std::wstring &majorVersion);
	VOID  GetAtaMinorVersion(WORD w81, std::wstring &minor);
	...
	static int Compare(const ATA_SMART_INFO& p1, const ATA_SMART_INFO& p2);

	std::wstring GetModelSerial(std::wstring &model, std::wstring &serialNumber);
