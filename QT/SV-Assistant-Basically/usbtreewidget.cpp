...
usbtreewidget::usbtreewidget(QWidget *parent) : QTreeWidget(parent)
{
...
void usbtreewidget::DEVICECHANGE()
{
	this->clear();
...
void usbtreewidget::CreateUsbTree()
{
    ...
    this->addItem(QStringList(rootkey), tr(""), rootkey, QIcon(tr(":/Usb/Usb/monitor.ico")));
    for(auto& i : SV_ASSIST::USB::GetAllUsbinfo())
    {
        int a = 0;
        ...
        qDebug() << thekey;
        this->addItem(QStringList(t), parentKey, thekey, QIcon(tr(":/Usb/Usb/usb.ico")));
        ...
        {
            int b = 0;
            for(const auto& p : j.UsbExternalHubInfo)
            {
                QString parenthubKey = thekey;
                QString thehubkey = QString::fromStdWString(p.HubName).replace("\\", "_");
                this->addItem(QStringList(tr("Port ") + QString::number(a++)+ tr(" ") + QString::fromStdWString(p.UsbDeviceProperties.DeviceDesc)), parenthubKey, thehubkey, QIcon(tr(":/Usb/Usb/hub.ico")));
                this->SaveUsbInfo(thehubkey, p.DeviceInfoType, nullptr, nullptr, &p);
                for(const auto& l : j.UsbDeviceInfo)
                {
                    ...
                    {
                        this->addItem(QStringList(tr("Port ") + QString::number(b)+ tr(" ") + QString::fromStdWString(l.UsbDeviceProperties.DeviceDesc)), parentportKey, theportkey, QIcon(tr(":/Usb/Usb/usb.ico")));
                    }
                    else
                    {
                        this->addItem(QStringList(tr("Port ") + QString::number(b)), parentportKey, theportkey, QIcon(tr(":/Usb/Usb/port.ico")));
                    }
        ...
        for(auto& k : i.UsbDeviceInfo.UsbDeviceInfo)
        {
            ...
            if(k.ConnectionInfo.ConnectionStatus == DeviceConnected)
            {
                ...
                this->addItem(QStringList(tr("Port ") + QString::number(a) + tr(" ") + QString::fromStdWString(k.UsbDeviceProperties.DeviceDesc)), parentportKey, theportkey, QIcon(tr(":/Usb/Usb/usb.ico")));
            }
            else
            {
                this->addItem(QStringList(tr("Port ") + QString::number(a)), parentportKey, theportkey, QIcon(tr(":/Usb/Usb/port.ico")));
            }
...
bool usbtreewidget::DelDir(const QString &path)
{
	if (path.isEmpty()) {
		return false;
	}
