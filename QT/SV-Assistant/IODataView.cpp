...
IODataView::IODataView(QWidget *parent)
	: QTableView(parent), model(new QStandardItemModel()),Base_Addr(0)
{
	this->Init();
	this->setModel(model);
}
...
void IODataView::Init()
{
	...
	QStringList horlist;
	horlist << tr("00") << tr("01") << tr("02") << tr("03") << tr("04") << tr("05") << tr("06") << tr("07") << tr("08") << tr("09") << tr("0A") << tr("0B") << tr("0C") << tr("0D") << tr("0E") << tr("0F");
	QStringList verlist;
	verlist << tr("00") << tr("10") << tr("20") << tr("30") << tr("40") << tr("50") << tr("60") << tr("70") << tr("80") << tr("90") << tr("A0") << tr("B0") << tr("C0") << tr("D0") << tr("E0") << tr("F0");
	for (int i = 0; i < 16; i++)
	{
		model->setHorizontalHeaderItem(i, new QStandardItem(horlist.at(i)));
		model->setVerticalHeaderItem(i, new QStandardItem(verlist.at(i)));
	...
	for (int i = 0; i < 0x10; i++)
	{
		for (int j = 0; j < 0x10; j++)
		{
			...
			std::shared_ptr<QStandardItem> temp = std::make_shared<QStandardItem>(QString::number(iodata).sprintf("%02X", iodata));
			model->setItem(i, j, temp.get());
