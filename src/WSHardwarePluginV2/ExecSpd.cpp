...
const std::string Hardware::Memory::SPDManufacturer(const BYTE Bank, const BYTE IDCode)
{
...
void ExecMemoryDDR3::Execute(MemoryCommonInformation& data)
{
	ExecMemory::Execute(data);

	if (spd.DIMM_Detail.MTB_Dividend == 1 && spd.DIMM_Detail.MTB_Divisor == 8)
	{
		if (spd.DIMM_Detail.sTCKmin == 10)
			data.MaxBandWidth = "DDR3-1600 (800MHz)";
		else if (spd.DIMM_Detail.sTCKmin == 12)
			data.MaxBandWidth = "DDR3-1333 (666.67MHz)";
		else if (spd.DIMM_Detail.sTCKmin == 15)
			data.MaxBandWidth = "DDR3-1066 (533MHz)";
		else if (spd.DIMM_Detail.sTCKmin == 20)
			data.MaxBandWidth = "DDR3-800 (400MHz)";
	}
	int  Ranks = spd.DIMM_Detail.sModuleOrganization.bits.Number_of_Ranks, Primarybuswidth = spd.DIMM_Detail.sBusWidth.bits.Primary_bus_width, \
		SDRAMWidth = spd.DIMM_Detail.sModuleOrganization.bits.SDRAM_Depth_Width;
	double capacity = 0;
	Ranks++;
	switch (spd.DIMM_Detail.sSDRAMBank_Density.bits.Capacity)
	{
	case 0:
		capacity = 0.25;
		break;
	...
	data.ModuleSize = DDRSize;

	{
		int Banks = 1 << (spd.DIMM_Detail.sSDRAMBank_Density.bits.Bank + 3);
		char temp[40];
		snprintf(temp, 40, "%d Ranks %d Banks", Ranks, Banks);
		data.Ranks_Banks = std::string(temp);
	}

	// 	if (!spd.DIMM_Detail.sModuleNominalVoltage.bits.VDD_1_5)
	// 		data.Voltages += " 1.5 V";
	// 	if (spd.DIMM_Detail.sModuleNominalVoltage.bits.VDD_1_35)
	// 		data.Voltages += " 1.35 V";
	// 	if (spd.DIMM_Detail.sModuleNominalVoltage.bits.VDD_1_2X)
	// 		data.Voltages += " 1.2X V";
	//
	// 	double FTB = spd.DIMM_Detail.sFTB.bits.FTB_Dividend * 1.0 / spd.DIMM_Detail.sFTB.bits.FTB_Divisor;
	// 	double MTB = spd.DIMM_Detail.MTB_Dividend * 1.0 / spd.DIMM_Detail.MTB_Divisor;
	// 	data.tCKmin = MTB * spd.DIMM_Detail.sTCKmin;
	// 	data.tAAmin = MTB * spd.DIMM_Detail.sTAAmin;
	// 	data.tRCDmin = MTB * spd.DIMM_Detail.sTRCDmin;
	// 	data.tRPmin = MTB * spd.DIMM_Detail.sTRPmin;
	// 	data.tRASmin = MTB * ((spd.DIMM_Detail.sUpper_TRAS_TRCmin.bits.tRASmin_Most << 8) | spd.DIMM_Detail.sTRASminLeast);
	// 	data.tWRmin = MTB * spd.DIMM_Detail.sTWRmin;
	// 	data.tRRDmin = MTB * spd.DIMM_Detail.sTRRDmin;
	// 	data.tRCmin = MTB * ((spd.DIMM_Detail.sUpper_TRAS_TRCmin.bits.tRCmin_Most << 8) | spd.DIMM_Detail.sTRCminLeast);
	// 	data.tRFCmin = MTB * ((spd.DIMM_Detail.sTRFCminMost << 8) | spd.DIMM_Detail.sTRFCminLeast);
	// 	data.tWTRmin = MTB * spd.DIMM_Detail.sTWTRmin;
	// 	data.tRTPmin = MTB * spd.DIMM_Detail.sTRTPmin;
	// 	data.tFAWmin = MTB * ((spd.DIMM_Detail.sTFAWminMost.bits.tFAWmin_Most << 8) | spd.DIMM_Detail.sTFAWminLeast);
}

void ExecMemoryDDR4::Execute(MemoryCommonInformation& data)
{
	ExecMemory::Execute(data);
	int Primarybuswidth = spd.DIMM_Detail.sBusWidth.bits.Primary_bus_width, SDRAMWidth = spd.DIMM_Detail.sModuleOrganization.bits.SDRAM_Depth_Width, \
		Ranks = spd.DIMM_Detail.sModuleOrganization.bits.Number_of_Ranks_per_DIMM;
	double capacity = 0;
	Ranks++;
	switch (spd.DIMM_Detail.sSDRAMBank_Density.bits.Capacity)
	{
	...
	if (spd.DIMM_Detail.sPackageType.bits.SignalLoading == 2)
		Ranks = Ranks * spd.DIMM_Detail.sPackageType.bits.DieCount;
	double DDRSize = capacity / 8.0 * Primarybuswidth / SDRAMWidth * Ranks;
	data.ModuleSize = DDRSize;

	if (spd.DIMM_Detail.sTCKmin == 10 && spd.DIMM_Detail.sTCKmin125 == 0)
		data.MaxBandWidth = "DDR4-1600 (800MHz)";
	else if (spd.DIMM_Detail.sTCKmin == 9 && spd.DIMM_Detail.sTCKmin125 == 0xCA)
		data.MaxBandWidth = "DDR4-1866 (933MHz)";
	else if (spd.DIMM_Detail.sTCKmin == 8 && spd.DIMM_Detail.sTCKmin125 == 0xC1)
		data.MaxBandWidth = "DDR4-2133 (1066.67MHz)";
	else if (spd.DIMM_Detail.sTCKmin == 7 && spd.DIMM_Detail.sTCKmin125 == 0xD6)
		data.MaxBandWidth = "DDR4-2400 (1200MHz)";
	else if (spd.DIMM_Detail.sTCKmin == 6 && spd.DIMM_Detail.sTCKmin125 == 0)
		data.MaxBandWidth = "DDR4-2666 (1333.33MHz)";
	else if (spd.DIMM_Detail.sTCKmin == 5 && spd.DIMM_Detail.sTCKmin125 == 0)
		data.MaxBandWidth = "DDR4-3200 (1600MHz)";
	{
		int BankGroup = 1;
		switch (spd.DIMM_Detail.sSDRAMBank_Density.bits.BankGroupBits)
		{
		case 1:
			BankGroup = 2;
			break;
		...
		switch (spd.DIMM_Detail.sSDRAMBank_Density.bits.BankAddressBits)
		{
		case 0:
			BankAddressBits = 4;
			break;
		...
		char temp[40];
		snprintf(temp, 40, "%d Ranks %d Banks", Ranks, BankAddressBits * BankGroup);
		data.Ranks_Banks = std::string(temp);
	}
	// 	data.Voltages = "1.2 V";
	// 	double FTB = 0.001, MTB = 0.125;
	// 	data.tCKmin = spd.DIMM_Detail.sTCKmin * MTB;
	// 	data.tAAmin = spd.DIMM_Detail.sTAAmin * MTB;
	// 	data.tRCDmin = MTB * spd.DIMM_Detail.sTRCDmin;
	// 	data.tRPmin = MTB * spd.DIMM_Detail.sTRPmin;
	// 	data.tRASmin = MTB * ((spd.DIMM_Detail.sUpper_TRAS_TRCmin.bits.tRASmin_Most << 8) | spd.DIMM_Detail.sTRASminLeast);
	// 	data.tWRmin = MTB * spd.DIMM_Detail.sTWRmin;
	// 	data.tRRDmin = MTB * spd.DIMM_Detail.sTRRD_Smin;
	// 	data.tRCmin = MTB * ((spd.DIMM_Detail.sUpper_TRAS_TRCmin.bits.tRCmin_Most << 8) | spd.DIMM_Detail.sTRCminLeast);
	// 	data.tRFCmin = MTB * ((spd.DIMM_Detail.sTRFC1minMost << 8) | spd.DIMM_Detail.sTRFC1minLeast);
	// 	data.tWTRmin = MTB * spd.DIMM_Detail.sTWRmin;
	// 	data.tRTPmin = 0.0;
	// 	data.tFAWmin = MTB * ((spd.DIMM_Detail.sTFAWminMost.bits.tFAWmin_Most << 8) | spd.DIMM_Detail.sTFAWminLeast);
}